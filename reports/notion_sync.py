...
Reads completed (unsynced) sessions from per-kid Notion timer databases,
inserts them into the PostgreSQL activity_logs table, and marks them as synced.
...
def query_unsynced_entries(db_id, target_date=None):
    """Query a Notion database for unsynced, completed (Done=true) entries."""
...
def insert_activity_log(conn, record):
    """Insert one record into activity_logs. Returns the new log_id."""
    sql = """
...
def mark_synced(page_id):
    """Set Synced=true on a Notion page."""
...
def main():
    ...
    parser.add_argument("--all", action="store_true",
                        help="Sync all unsynced entries regardless of date")
    ...
    try:
        for child_id, db_id in NOTION_DB_IDS.items():
            who = CHILDREN[child_id]
            print(f"Querying {who}'s timer for unsynced entries ({date_label})...")

            entries = query_unsynced_entries(db_id, target_date)
            print(f"  Found {len(entries)} unsynced entries.")
